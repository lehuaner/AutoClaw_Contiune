...
class App:
    ...
    def __init__(self, root):
        ...
        root.title("AutoClaw 自动继续  · 适配 AutoClaw %s" % AUTOCLAW_VERSION)
    ...
    def _summarize_params(self):
        ...
        return "".join("  %s = %s\n" % (k, v) for k, v in (
            ("适配 AutoClaw 版本", AUTOCLAW_VERSION),
    ...
    def toggle(self):
        if not self.running:
            ...
            self._append_log("已适配 AutoClaw %s，开始监控日志：%s" %
                             (AUTOCLAW_VERSION, self.log_path_abs))
    ...
    def _click_session_button(self, hwnd, agent, session):
        ...
        try:
            ...
            # 优先用 UIA InvokePattern 点击（不依赖坐标/前台，最可靠，实测有效）
            try:
